...
export const createTodo = (todoText: string) => {
  if (!todoText) return;
  todos.value = [...todos.peek(), { id: nanoid(), content: todoText, completed: false }];
};

export const updateTodoContent = (patch: Pick<TodoItem, "id" | "content">) => {
  todos.value = todos.peek().map((e) => (e.id === patch.id ? { ...e, ...patch } : e));
};

export const changeTodoCompletedById = (id: string) => {
  todos.value = todos.peek().map((e) => (e.id === id ? { ...e, completed: !e.completed } : e));
};
